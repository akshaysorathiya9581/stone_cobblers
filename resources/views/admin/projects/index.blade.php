                    ...
                    <tbody>
                        @foreach ($projects as $project)
                            <tr data-status="{{ strtolower($project->status) }}">
                                ...
                                {{-- Actions --}}
                                <td class="actions">
                                    <a href="{{ route('admin.projects.show', $project) }}" class="action-btn view"
...
@push('scripts')
    ...
    <script>
        $(function () {
            const $tabs = $('.tabs .tab'),
                $rows = $('.projects-table table tbody tr'),
                $empty = $('.no-records');
            ...
            function filterRows(status) {
                ...
                $rows.toggle(status === 'all' || $rows.map((_, r) => $(r).data('status')).get().includes(status));
                $rows.each(function () {
                    $(this).toggle(status === 'all' || normalize($(this).data('status')) === status);
                });
            ...
            $tabs.on('click', function () {
                const $tab = $(this),
                    status = $tab.data('status') || normalize($tab.text());
            ...
            const saved = localStorage.getItem('projectTab') || 'all';
            $tabs.filter(`[data-status="${saved}"]`).first().addClass('active').trigger('click');
        });
    </script>
