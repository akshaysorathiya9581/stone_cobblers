...
@push('css')
    <style>
        ...
        .btn { display:inline-flex; align-items:center; gap:.4rem; text-decoration:none; cursor:pointer; }
                ...
                <form id="customerForm" novalidate>
                    ...
                    <div class="form-fields">
                        ...
                        <div class="form-field">
                            ...
                            <input type="text" class="form-input" id="first_name" name="first_name" placeholder="Enter your first name" required>
                            <small class="error-text" data-for="first_name"></small>
                        </div>
                        <div class="form-field">
                            <label class="form-label required" for="last_name">Last Name</label>
                            <input type="text" class="form-input" id="last_name" name="last_name" placeholder="Enter your last name" required>
                    ...
                    <div class="form-fields">
                        ...
                        <div class="form-field">
                            ...
                            <input type="email" class="form-input" id="email" name="email" placeholder="your.email@example.com" required>
                    ...
                    <div class="form-fields">
                        ...
                        <div class="form-field">
                            ...
                            <input type="text" class="form-input" id="address" name="address" placeholder="123 Main Street" required>
                            <small class="error-text" data-for="address"></small>
                        </div>
                        <div class="form-field">
                            <label class="form-label required" for="city">City</label>
                            <input type="text" class="form-input" id="city" name="city" placeholder="Your City" required>
                            <small class="error-text" data-for="city"></small>
                        </div>
                        <div class="form-field">
                            <label class="form-label required" for="state">State</label>
                            <input type="text" class="form-input" id="state" name="state" placeholder="Your State" required>
                            <small class="error-text" data-for="state"></small>
                        </div>
                        <div class="form-field">
                            <label class="form-label required" for="zipCode">ZIP Code</label>
                            <input type="text" class="form-input" id="zipCode" name="zipCode" placeholder="12345" required>
                    ...
                    <div class="form-fields">
                        <div class="field-row">
                          <div class="form-field single">
                              ...
                              <textarea class="form-input" id="additionalNotes" name="additionalNotes" rows="4" placeholder="Any special requirements, preferences, or questions..."></textarea>
							...
							<div class="form-field">
								...
								<select name="customer_status" id="customer_status" class="form-input">
									...
									@foreach(get_customer_status_list() as $status)
										<option value="{{ $status['id'] }}" @if(old('customer_status') == $status['id']) selected @endif>
											{{ $status['text'] }}
										</option>
									@endforeach
								</select>
                    ...
                    <div style="text-align:center;padding:40px;">
                        <div class="form-icon icon-check" style="background-color:rgb(22,163,74);color:#fff;border-color:rgb(22,163,74);">✓</div>
                        <h1 class="form-title" style="color:rgb(22,163,74);margin:20px 0;">Customer Added Successfully!</h1>
                        <p class="form-subtitle" style="margin-bottom:30px;">Thank you for choosing The Stone Cobblers. We'll be in touch soon!</p>
                        <a href="{{ route('admin.customers.index') }}" class="nav-btn next" style="text-decoration:none;display:inline-flex;"><i>🏠</i> Back to Customers</a>
                    </div>
                    </div>
...
(function($){
  $(function(){
    ...
    function validateCurrentStep(){
      ...
      $required.each(function(){
        ...
        if ($el.attr('type') === 'email'){
          ...
          tmp.type = 'email'; tmp.value = val;
          if (!tmp.checkValidity()){
            ok = false; setError($el, 'Please enter a valid email address.');
            return;
          }
        }
        clearError($el);
    ...
    function fillReview(){
      ...
      const $fields = $('.form-input').filter(function(){ return !!$(this).attr('name'); });
    ...
    async function checkEmailUnique(email){
      ...
      const tmp = document.createElement('input');
      tmp.type='email'; tmp.value=key;
      if (!tmp.checkValidity()){
        emailChecked = true; emailUnique = false; emailMessage = 'Please enter a valid email address.';
        return {unique:false, message:emailMessage};
      }

      if (emailCache[key] !== undefined){
        const { unique, message } = emailCache[key];
        emailChecked = true; emailUnique = unique; emailMessage = message || '';
      ...
      try{
        ...
        emailChecked = true; emailUnique = !!res.unique; emailMessage = res.message || '';
        return emailCache[key];
      }catch(e){
        // If the check fails (network), don't hard-block. Treat as unknown/ok.
        emailChecked = true; emailUnique = true; emailMessage = '';
    ...
    async function submitFormAjax(){
      ...
      try{
        ...
        if (xhr.status === 422){
          ...
          if (firstField){
            const idx = $steps.toArray().findIndex(sec => $(sec).find(`[name="${firstField}"]`).length);
