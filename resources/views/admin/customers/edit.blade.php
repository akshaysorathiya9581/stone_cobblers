        ...
        <div class="content bg-content">
            <div class="form-container">
                ...
                <form id="customerForm" novalidate
                      data-mode="{{ isset($customer) ? 'edit' : 'create' }}"
                ...
                >
                            ...
                            <div class="field-row">
                                ...
                                <div class="form-field">
                                    ...
                                    <select class="form-input custom-select" id="referralSource" name="referralSource" data-placeholder="Select How did you hear about us?">
                                        <option></option>
                                        <option value="Google Search" {{ old('referralSource', $customer->referralSource ?? '') == 'Google Search' ? 'selected' : '' }}>Google Search</option>
                                        <option value="Social Media" {{ old('referralSource', $customer->referralSource ?? '') == 'Social Media' ? 'selected' : '' }}>Social Media</option>
                                        <option value="Referral" {{ old('referralSource', $customer->referralSource ?? '') == 'Referral' ? 'selected' : '' }}>Referral</option>
                                        <option value="Advertisement" {{ old('referralSource', $customer->referralSource ?? '') == 'Advertisement' ? 'selected' : '' }}>Advertisement</option>
                                        <option value="Other" {{ old('referralSource', $customer->referralSource ?? '') == 'Other' ? 'selected' : '' }}>Other</option>
                                    </select>
                    ...
                    <div class="form-container-view" id="success-step">
                        <div style="text-align:center;padding:40px;">
                            ...
                            <h1 class="form-title" id="success-title" style="color:rgb(22,163,74);margin:20px 0;">Customer updated Successfully!</h1>
                            <p class="form-subtitle" id="success-msg" style="margin-bottom:30px;">Thank you for choosing The Stone Cobblers. We'll be in touch soon!</p>
    ...
    <script>
        ...
        (function($) {
            $(function() {
                ...
                let emailMessage = '';
                const emailCache = {}; // cache by email+id key
                ...
                async function checkEmailUnique(email) {
                    ...
                    try {
                        const res = await $.ajax({
                            ...
                            data: JSON.stringify({ email: String(email || '').trim(), id: id })
                        });
                ...
                async function submitFormAjax() {
                    ...
                    try {
                        let url = `{{ route('admin.customers.store') }}`; // default create
                        let method = 'POST';
